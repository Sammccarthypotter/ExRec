...
def printOutFile(finalFile,outputFilePath,locus):
   ...
   with open(outputFilePath, 'w') as f:
           ...
           print("#NEXUS")
           print("begin data;")
           print("\tdimensions ntax=" + str(finalFile.numberOfspecies) + " nchar=" + str(ncha) + ";")
...
class NexusFile:
 ...
 def __init__ (self, inputFilePath):
   ...
   for i in range(len(LociName)):
    if not LociName[i].split(".")[-1] == ".nex":
      LociName[i] = LociName[i].split(".")[0] + ".nex"
...
if __name__ == "__main__":
  ...
  outputFolder = "nexus_split_files"
